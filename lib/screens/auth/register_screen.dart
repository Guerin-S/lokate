...
class _RegisterScreenState extends State<RegisterScreen> {
  ...
  Future<void> _register() async {
    ...
    try {
      ...
      setState(() => _error = 'Inscription échouée. Vérifiez vos informations.');
    } finally {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Créer un compte'), leading: BackButton(onPressed: () => context.pop())),
          ...
          child: Column(
            ...
            children: [
              ...
              Container(
                ...
                child: Row(
                  children: [
                    Expanded(child: _TabButton(label: 'Email', selected: _authMethod == 0, onTap: () => setState(() => _authMethod = 0))),
                    Expanded(child: _TabButton(label: 'Téléphone', selected: _authMethod == 1, onTap: () => setState(() => _authMethod = 1))),
                  ],
                ),
              ),
              ...
              LokateTextField(controller: _nameCtrl, label: 'Nom complet', hint: 'Jean Dupont', prefixIcon: Icons.person_outlined,
                validator: (v) => v!.length > 2 ? null : 'Nom trop court'),
              const SizedBox(height: 14),

              if (_authMethod == 0) ...[
                LokateTextField(controller: _emailCtrl, label: 'Email', hint: 'votre@email.com',
                  keyboardType: TextInputType.emailAddress, prefixIcon: Icons.email_outlined,
                  validator: (v) => v!.contains('@') ? null : 'Email invalide'),
                const SizedBox(height: 14),
                LokateTextField(controller: _passwordCtrl, label: 'Mot de passe', hint: '••••••••',
                ...
                LokateTextField(controller: _phoneCtrl, label: 'Numéro de téléphone', hint: '6XX XXX XXX',
                  keyboardType: TextInputType.phone, prefixIcon: Icons.phone_outlined,
                  prefix: const Text('+237 ', style: TextStyle(fontWeight: FontWeight.w600, color: AppColors.primary)),
                  validator: (v) => v!.length == 9 ? null : '9 chiffres requis'),
              ],
              ...
              ElevatedButton(
                ...
                child: _loading
                    ...
                    : Text(_authMethod == 0 ? 'Créer mon compte' : 'Envoyer le code SMS'),
              ),
            ],
          ),
        ),
...
class _RoleCard extends StatelessWidget {
  ...
  const _RoleCard({required this.emoji, required this.label, required this.selected, required this.onTap});
